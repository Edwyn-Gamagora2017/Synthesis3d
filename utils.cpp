...
void save_img(std::string nom, int w, int h, pixel ** couleurs) {
	std::fstream writer;
	writer.open( std::string(nom+".ppm").c_str(), std::fstream::out);
	writer << "P3\n";
	writer << w << " " << h << "\n";
	writer << "255\n";
...
vec3 * couleurToAlbedo(vec3 couleur) {
	return new vec3( couleurToAlbedo(couleur.getX()), couleurToAlbedo(couleur.getY()), couleurToAlbedo(couleur.getZ()));
}
double albedoToCouleur(double albedo) {
	return clamp( albedo * 255., 0, 255 );
}
